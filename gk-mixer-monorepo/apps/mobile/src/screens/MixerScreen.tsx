...
import BeakerView from '../components/BeakerView';
...
export default function MixerScreen() {
  ...
  const ralMatch = useMemo(() => findNearestRAL(colorData.rgb), [colorData]);
  const nearestPaints = useMemo(() => findNearestPaints(colorData.hex, 3), [colorData.hex]);
  ...
  const layers = useMemo(() => {
    return mixRatios
      .map((r, i) => ({
        ratio: r,
        color: EXTENDED_MIXING_COLORS[i].hex,
        label: EXTENDED_MIXING_COLORS[i].name,
  ...
  const { hex, rgb, cmyk, hsb, lab } = colorData;
    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <View style={styles.section}>
        ...
        <View style={styles.volumeRow}>
          {VOLUMES.map((v) => (
            ...
            >
              <Text style={[styles.volumeText, bottleVolume === v && styles.volumeTextActive]}>{v}ml</Text>
            </TouchableOpacity>
          ))}
        </View>
      </View>
      ...
      {layers.length > 0 && (
        <View style={styles.section}>
          ...
          <BeakerView layers={layers} width={240} height={340} />
        </View>
      )}
...
const styles = StyleSheet.create({
  ...
  inputRow: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#1A1A1A', borderRadius: 12, paddingHorizontal: 16, marginBottom: 16 },
